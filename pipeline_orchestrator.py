...
def get_plan_date(plan: Any) -> Optional[str]:
    """Extracts or parses the normalized YYYY-MM-DD date for a plan summary or mock."""
    plan_date = getattr(plan, "date", None)
    if isinstance(plan_date, str):
        return plan_date
    for attr in ("dates_raw", "dates"):
        val = getattr(plan, attr, None)
        if isinstance(val, str):
            return parse_pco_plan_date(val)
    return None
...
def prompt_for_plan(
    ...
    """Provides an interactive CLI menu to select Planning Center plan(s).
    
    If target_date is provided, filters for plans matching that specific date.
    If start_date and end_date are provided, filters for plans within that date range.
    Otherwise, lists the most recent plans.
    ...
    if target_date:
        matching_plans = []
        for plan in plans:
            p_date = get_plan_date(plan)
            if p_date == target_date:
                matching_plans.append((plan, target_date))
                
        if not matching_plans:
            print(f"❌ No Planning Center plans found for date '{target_date}'.")
            sys.exit(1)
            
        print(f"\n📋 Planning Center Services for {target_date}:")
        print("-" * 40)
        for idx, (plan, _) in enumerate(matching_plans, start=1):
            title_str = f" - {plan.title}" if getattr(plan, "title", None) else ""
            display_date = getattr(plan, "dates_raw", None) or getattr(plan, "dates", "Unknown Date")
            print(f"  [{idx}] {display_date}{title_str}")
        if len(matching_plans) > 1:
            print(f"  [a] All services on this date ({len(matching_plans)} plans)")
        print("-" * 40)
        
        prompt_label = f"Select a service [1-{len(matching_plans)}"
        if len(matching_plans) > 1:
            prompt_label += " or 'a' for all"
        prompt_label += "] (Default: 1): "
        
        while True:
            try:
                choice = input(prompt_label).strip()
                if choice in ("", "1"):
                    selected, formatted_date = matching_plans[0]
                    return selected.id, formatted_date
                if choice.lower() in ("a", "all") and len(matching_plans) > 1:
                    return [(p.id, d) for p, d in matching_plans]
                selected_idx = int(choice) - 1
                if 0 <= selected_idx < len(matching_plans):
                    selected, formatted_date = matching_plans[selected_idx]
                    return selected.id, formatted_date
                else:
                    print("Invalid selection. Please try again.")
            except ValueError:
        ...
        for plan in plans:
            ...
            if p_date and start_date <= p_date <= end_date:
                matching_plans.append((plan, p_date))
                
        if not matching_plans:
            print(f"❌ No Planning Center plans found between {start_date} and {end_date}.")
            sys.exit(1)
            
        print(f"\n📋 Planning Center Services between {start_date} and {end_date}:")
        print("-" * 40)
        for idx, (plan, _) in enumerate(matching_plans, start=1):
            title_str = f" - {plan.title}" if getattr(plan, "title", None) else ""
            display_date = getattr(plan, "dates_raw", None) or getattr(plan, "dates", "Unknown Date")
            print(f"  [{idx}] {display_date}{title_str}")
        if len(matching_plans) > 1:
            print(f"  [a] All services in this range ({len(matching_plans)} plans)")
        print("-" * 40)
        ...
        while True:
            try:
                ...
                if 0 <= selected_idx < len(plans):
                    ...
                    formatted_date = get_plan_date(selected)
    ...
    try:
        if run_main_pipeline:
            ...
            for current_plan_id, current_date in target_plans:
                ...
                plan = fetch_service_plan(service_type, current_plan_id, current_date)
                ...
                if args.audio_file:
                    ...
                    try:
                        discovered_paths = discover_raw_audio(current_date, RAW_AUDIO_DIR)
                    ...
                    if discovered_paths:
                        ...
                        print(f"❌ Error: Could not find any raw audio files matching '{current_date}' in {RAW_AUDIO_DIR}.")
